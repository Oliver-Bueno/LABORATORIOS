...
class Reproductor():
    def __init__(self):
        ...
        # Canvas and graphical elements
        self.canvas = tk.Canvas(self.ventana, width=850, height=480, background="black")
        self.canvas.place(relx=0.5, rely=0.5, anchor="center")

        # Constructing absolute paths for images
        script_dir = os.path.dirname(__file__)  # Directory where the script is located
        azul_path = os.path.join(script_dir, 'imagenes', 'azul.png')
        negro_path = os.path.join(script_dir, 'imagenes', 'negro.png')
        audifonos1_path = os.path.join(script_dir, 'imagenes', 'audifonos1.png')
        play_path = os.path.join(script_dir, 'icons', 'control_play_blue.png')
        pause_path = os.path.join(script_dir, 'icons', 'control_pause_blue.png')
        stop_path = os.path.join(script_dir, 'icons', 'control_stop_blue.png')
        forward_path = os.path.join(script_dir, 'icons', 'control_fastforward_blue.png')
        rewind_path = os.path.join(script_dir, 'icons', 'control_rewind_blue.png')
        repeat_path = os.path.join(script_dir, 'icons', 'control_repeat_blue.png')

        self.img_fones = tk.PhotoImage(file=azul_path)
        self.img_fonem = tk.PhotoImage(file=negro_path)
        self.img_foneb = tk.PhotoImage(file=azul_path)
        ...
        # Icons
        iconoPlay = tk.PhotoImage(file=play_path)
        iconoPause = tk.PhotoImage(file=pause_path)
        iconoStop = tk.PhotoImage(file=stop_path)
        iconoAdelantar = tk.PhotoImage(file=forward_path)
        iconoDevolver = tk.PhotoImage(file=rewind_path)
        iconoAvanzar = tk.PhotoImage(file=repeat_path)
        iconoRetroceder = tk.PhotoImage(file=repeat_path)
        iconoAudifonos1 = tk.PhotoImage(file=audifonos1_path)
